...
- The release zip, such as `RGFrontend-H700-1.0.1.zip`.
...
2. Copy `RGFrontend-H700-1.0.1.zip` to the TF card root.

   Correct location example:

   ```text
   TF card root/RGFrontend-H700-1.0.1.zip
   TF card root/Roms/
   ```
...
```sh
scp RGFrontend-H700-1.0.1.zip root@10.1.1.233:/mnt/mmc/
ssh root@10.1.1.233
cd /mnt/mmc
unzip -o RGFrontend-H700-1.0.1.zip
```
...
```text
TF card root/RGFrontend-H700-1.0.1/Roms/APPS/...
```
